...
# Courier New at the same point size.
UI_FONT = "Consolas"
...
class MetricBar(QWidget):
    ...
    def paintEvent(self, _):
        ...
        p.setFont(QFont(UI_FONT, 11, QFont.Weight.Bold))
...
class _DropCanvas(QWidget):
    ...
    def _paint_drag_over(self, p, W, H):
        ...
        p.setFont(QFont(UI_FONT, 10, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.PRI), 1))
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for key, label in [("windows","⊞  Windows"),("mac","  macOS"),("linux","🐧  Linux")]:
            ...
            btn.setFont(QFont(UI_FONT, 11, QFont.Weight.Bold))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        # Cache the HWND for later use in foreground activation from bg threads
        self._hwnd = None
    ...
    def get_hwnd(self) -> int:
        """Get and cache the Windows HWND for this window."""
        if self._hwnd is None and _OS == "Windows":
            try:
                self._hwnd = int(self.winId())
                print(f"[MainWindow] HWND cached: {self._hwnd}")
            except Exception as e:
                print(f"[MainWindow] get_hwnd failed: {e}")
        return self._hwnd

    def showEvent(self, event):
        """Cache HWND when window is first shown."""
        super().showEvent(event)
        if _OS == "Windows" and self._hwnd is None:
            self.get_hwnd()

    def _on_wake_focus(self):
        """Slot: bring JARVIS to foreground and go fullscreen (runs on main thread via QueuedConnection)."""
        import threading as _th
        print(f"[WakeFocus] Slot running on thread: {_th.current_thread().name}")

        if _OS == "Windows":
            self._do_wake_windows()
        else:
    ...
    # ── Windows-specific aggressive activation ──────────────────────

    def _do_wake_windows(self):
        import ctypes
        ...
        hwnd = int(self.winId())
        print(f"[WakeFocus] Starting wake. minimized={self.isMinimized()}, state={self.windowState()}")

        # ── Phase 1: Make topmost + restore from minimized ──
        ...
        # ── Phase 2: Go fullscreen ──
        self.showFullScreen()
        self.repaint()
        QApplication.processEvents()

        # ── Phase 3: Remove topmost flag ──
        user32.SetWindowPos(hwnd, HWND_NOTOPMOST, 0, 0, 0, 0,
                            SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW)

        # ── Phase 4: Raise and activate ──
        self.raise_()
        self.activateWindow()
        QApplication.processEvents()

        # ── Phase 5: Win32 force foreground ──
        ...
        print(f"[WakeFocus] After Win32: fullscreen={self.isFullScreen()}, foreground={is_fg}")

        # ── Phase 7: Launch bring_to_front.py DETACHED (non-blocking) ──
        # os.system blocks the main thread → causes "not responding".
        # subprocess.Popen with DETACHED_PROCESS + close_fds launches it
        # independently so we don't block the Qt event loop.
        import sys as _sys
        script_path = BASE_DIR / "config" / "bring_to_front.py"
        DETACHED_PROCESS = 0x00000008
        print(f"[WakeFocus] Launching detached: {script_path}")
        try:
            subprocess.Popen(
            ...
            print(f"[WakeFocus] Detached process launched")
        except Exception as e:
            print(f"[WakeFocus] ❌ Popen failed: {e}")

        final_fg = user32.GetForegroundWindow() == hwnd
        print(f"[WakeFocus] Final: fullscreen={self.isFullScreen()}, foreground={final_fg}, active={self.isActiveWindow()}")
    ...
    def _build_header(self) -> QWidget:
        ...
        self._date_lbl.setFont(QFont(UI_FONT, 9))
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self._uptime_lbl.setFont(QFont(UI_FONT, 10, QFont.Weight.Bold))
        self._uptime_lbl.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        ip_lay.addWidget(self._uptime_lbl)

        self._proc_lbl = QLabel("PROC  --")
        self._proc_lbl.setFont(QFont(UI_FONT, 10))
        self._proc_lbl.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent; border: none;")
        ip_lay.addWidget(self._proc_lbl)

        os_name = {"Windows": "WIN", "Darwin": "macOS", "Linux": "LINUX"}.get(_OS, _OS.upper())
        os_lbl = QLabel(f"OS  {os_name}")
        os_lbl.setFont(QFont(UI_FONT, 10))
        ...
        for txt, col in [
            ...
            lbl = QLabel(txt)
            lbl.setFont(QFont(UI_FONT, 9, QFont.Weight.Bold))
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        def _sec(txt):
            l = QLabel(f"▸ {txt}")
            l.setFont(QFont(UI_FONT, 9, QFont.Weight.Bold))
        ...
        self._mute_btn.setFont(QFont(UI_FONT, 10, QFont.Weight.Bold))
        ...
        fs_btn.setFont(QFont(UI_FONT, 9))
    ...
    def _build_footer(self) -> QWidget:
        ...
        def _fl(txt, color=C.TEXT_MED):
            l = QLabel(txt); l.setFont(QFont(UI_FONT, 9))
            l.setStyleSheet(f"color: {color}; background: transparent;")
            return l
...
class JarvisUI:
    ...
    def wake_and_focus(self):
        """Bring the JARVIS window to foreground and go fullscreen (thread-safe).
        Uses QueuedConnection — returns immediately, slot runs on main thread."""
        self._win._wake_focus_sig.emit()
